<script lang="ts">
	...
	async function handleSignUp(this: HTMLFormElement) {
...
<main class="w-full max-w-md p-8 | grid gap-12 | bg-dialog-color/30 lg:(max-w-4xl grid-cols-12)">
	...
	<div class="grid gap-7.5 | w-full | md:col-span-6 lg:(order-2)">
		<div class="grid gap-12">
			...
			<header>
				<h1 class="text-2xl text-white font-medium font-bold">Sign up</h1>
				<p class="text-sm">
		...
		<form class="grid gap-6" method="post" on:submit|preventDefault={handleSignUp}>
			<div class="grid gap-4.5">
				<Input id="name" minlength={4} maxlength={50} placeholder="John Doe" error={form?.name} />
				<Input
					id="display-name"
					label="username"
					minlength={4}
